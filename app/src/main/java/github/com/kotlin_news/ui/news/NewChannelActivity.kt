...
class NewChannelActivity : AppCompatActivity() {
    ...
    private val itemCilck = { p: Int, v: View, c: newChannel ->
        ...
        if (c.editAble) {
            ...
            doAsync {
                Thread.sleep(100)
                uiThread {
                    val addView = if (c.channelSelect) {
                        notShowLayoutManager.findViewByPosition(0)
                    } else {
                        val childSize = showLayoutManager.childCount
                        showLayoutManager.findViewByPosition(childSize - 1)
                    }
                    if (addView == null) {
                        window.dismiss()
                        return@uiThread
...
//                            var view = (if (c.channelSelect) notShowLayoutManager else showLayoutManager).findViewByPosition(index)
    ...
    private lateinit var showLayoutManager: GridLayoutManager
    private lateinit var notShowAdapter: NewChannelAdapter
    private lateinit var notShowLayoutManager: GridLayoutManager
    ...
    override fun finish() {
        val saveChannelList = RequestCommand.saveChannelList(showAdapter.newList)
        val saveChannelList1 = RequestCommand.saveChannelList(notShowAdapter.newList)
        super.finish()
    }
